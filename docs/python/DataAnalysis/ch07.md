...
```
##### 分组机制
分组操作第一步，数据包含在pandas对象中，可以是Series、DataFrame或其他数据结构。\
...
#### 数据聚合
聚合是指所有根据数组产生标量值的数据转换过程，比如：mean、count、min和sum等一些聚合操作。
```
import pandas as pd
import numpy as np
```
...
确定p分位数位置的两种方法(n为数据的总个数，p为0-1之间的值)。在python中计算分位数位置的方案采用position=1+(n-1)*p\
* position = (n+1)*p
* position = 1 + (n-1)*p
...
也可以通过向groupby传递group_keys=False来禁用这个功能。
```
...
##### 分位数与桶分析
第8章中，pandas有一些工具，尤其是cut和qcut，用于将数据按照你选择的箱位或样本分位数进行分桶。\
...
##### 示例：使用指定分组值填充缺失值
在清除缺失值时，有时会使用dropna来去除缺失值，有时使用修正值或来自于其他数据的值来输入（填充）到null值（NA）。\
fillna是一个可以使用的正确工具。\
...
##### 示例：分组加权平均和相关性
在groupby的拆分-应用-联合的范式下，DataFrame的列间操作或两个Seriese之间的操作，例如实现分组加权平均。\
...
import numpy as np
```
根据下面的小费数据集，计算一张在行方向上按day和smoker排列的分组平均值（默认的pivot_table聚合类型）的表。
...
# 4       24.59  3.61     No  Sun  Dinner     4  0.172069
```
计算在行方向上按day和smoker排列的分组平均值。也可以直接使用groupby实现
```
...
在tip_pct和size上进行聚合，并根据time分组。我将把smoker放入表的列，而将day放入表的行：
```

...
如果要迭代的是某种数据结构，例如字符串列表，那么可以直接在这个序列上面迭代，不需要通过`range`。
...
Python的内置的函数`enumerate`，能够把任何一种迭代器（iterator）封装成惰性生成器（lazy generator，参见Rule30）。
这样每次循环的时候，它只需要从iterator里面获取下一个值就行了，同时还会给出本轮循环的序号，即生成器每次产生的一对输出值。

下面通过内置的`next`函数手动推进`enumerate`所返回的这个iterator，来演示`enumerate`。

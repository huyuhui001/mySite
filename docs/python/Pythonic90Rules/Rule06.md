...
上面这段代码虽然没错，但看起来很乱，因为`snacks`结构本身并不是一份简单的列表，它的每个元素都是一个元组， 所以必须逐层访问才能查到最为具体的数据，也就是每种零食的名称（name）及卡路里（calories）。

下面换一种写法，首先调用内置的`enumerate`函数（参见第7条）获得当前要迭代的元组， 然后针对这个元组做unpacking，这样就可以直接得到具体的`name`与`calories`值了。

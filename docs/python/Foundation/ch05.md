...
在一个类中以另一个类的对象作为数据属性，称为类的 **组合（Class Combination）**。组合与继承都是用来解决代码的重用性问题。
...
再举一个例子，`Car`类包含`Engine`类的实例，因为`Car`有一个`Engine`。在`Car`类定义中通过`self.engine = engine`来实现访问`Engine`实例的方法。
...
再举一个例子，假设我们有一个`Book`类和一个`Library`类。`Library`类包含多个`Book`类的实例，因为`Library`有多个`Book`。

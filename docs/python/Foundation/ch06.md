...
- 模块入口点。当导入一个Python模块或包时，它会被赋予一个特殊的变量`__name__`。下例中，`print(offsets.__name__)`输出的是`offsets.py`这个模块自己的`__name__`值，即`offsets`模块的完全限定名称。
- 这个变量的值为模块或包的完全限定名称，也是导入系统看到的名称。但在直接运行时，`__name__`会被设置为 `"__main__"`。下例中`print(__name__)`输出的是运行时赋给`__name__`的值`"__main__"`。

```python
from pandas.tseries import offsets

print(__name__)
print(offsets.__name__)

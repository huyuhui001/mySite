...
1．在Shell窗口的提示符下使用`dir`和`help`函数来探索Python的内置多项集类型`str`、`list`、`tuple`、`set`以及`dict`的接口。它们的语法是`dir(<type name>)`和`help(<type name>)`。
...
2．查看`java.util`包里所提供的Java多项集类型，并和Python的多项集类型加以比较。

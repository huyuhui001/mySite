...
与数组相比，单向链接结构的主要优势并不在于时间复杂度而在于内存性能。
调整数组尺寸（在需要的时候执行），使其在时间和内存上都是线性的；调整链接结构的大小（会在插入或删除时发生）在时间和内存上都是常数复杂度。

在链接结构里，不会有内存的浪费，因为这个结构的物理尺寸永远都不会超过逻辑尺寸。

链接结构的确会产生一些额外的内存开销，这是因为单向链接结构必须要有额外的`n`个内存单元来存储指针。

对于双向链接结构来说，它的节点里包含两个链接，因此内存的成本会更高一些。
